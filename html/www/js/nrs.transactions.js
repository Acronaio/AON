...
var NRS = (function(NRS, $, undefined) {
				...
				if (responsePoll.transaction) {
					NRS.sendRequest("getPhasingPollVote", {
						...
						if (vm == 1) {
							$popoverTypeTR.find("td:first").html($.t('accounts', 'Accounts') + ":");
							$popoverTypeTR.find("td:last").html(String(attachment.phasingWhitelist ? attachment.phasingWhitelist.length : ""));
							votesFormatted = NRS.convertToNXT(responsePoll.result) + " / " + NRS.convertToNXT(attachment.phasingQuorum) + " AON";
							$popoverVotesTR.find("td:last").html(votesFormatted);
						}
						if (mbModel == 1) {
							if (minBalance > 0) {
								minBalanceFormatted = NRS.convertToNXT(minBalance) + " AON";
    ...
    NRS.getTransactionRowHTML = function(t, actions, decimals, isScheduled) {
		...
		html += "<td class='tx-logo-td'>";
		html += "<div><span class='tx-logo-circle'><img src='img/tx-logo.svg'/></span>";
		html += "</td>"
		html += "<td>";
		html += "<p class='tx-from db-text db-opacity-65'>From: " + ((NRS.getAccountLink(t, "sender") == "/" && t.type == 2) ? "Asset Exchange" : NRS.getAccountLink(t, "sender")) + "</p>";
		html += "<p class='tx-p db-text db-opacity-65'>To: " + ((NRS.getAccountLink(t, "recipient") == "/" && t.type == 2) ? "Asset Exchange" : NRS.getAccountLink(t, "recipient")) + " </p>";
		html += "<p class='tx-p db-text db-opacity-25'>" + NRS.formatTimestamp(t.timestamp) + "</p>";
		html += "</div>";
		html += "</td>";
		html += "<td>";
		html += "<p class='db-text db-opacity-65' style='float: right; margin: 8px 10px 0 0;'>" + formattedAmount + " AON</p>";
		html += "</td>";
		html += "</tr>";

    ...
    NRS.submitForm = function($modal, $btn) {
		...
		if (data.recipient) {
			...
			if (/^\d+$/.test(data.recipient)) {
				...
				NRS.unlockForm($modal, $btn);
				return;
			} else if (!/^AON\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(data.recipient)) {
				var convertedAccountId = $modal.find("input[name=converted_account_id]").val();
				if (!convertedAccountId || (!/^\d+$/.test(convertedAccountId) && !/^AON\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(convertedAccountId))) {

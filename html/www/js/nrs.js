...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.timeConverter = function(timestamp) {
    ...
    function initImpl() {
		...
		loadConstantsPromise.then(function() {
			...
			getStatePromise.then(function() {
				...
				$("#dgs_search_account_center").mask("AON-****-****-****-*****");
    ...
    NRS.showNews = function() {
    	if (NRS.news && NRS.news.length > 0) {
    		var newsRows = "";

    		NRS.news.forEach(el => {
				let ms = Date.parse(el.date_gmt);
				newsRows += "<tr>" +
					"<td class='tx-logo-td'\"'>" +
					"<div><span class='tx-logo-circle'><img src='img/tx-logo.svg'></span></div></td>" +
					"<td><a target='_blank' href='https://acrona.io/news/" + el.id + "'><p style='font-weight: bolder;' class='tx-from db-text db-opacity-65'>" + el.title.rendered +"</p></a>" +
					// "<p class='tx-p db-text db-opacity-65'>" + el.title.rendered + "</p></td>" +
				"<td><p class='db-text db-opacity-65' style='float: right; margin: 8px 10px 0 0;'>" +  NRS.timeConverter(ms) + "</p> </td></tr>";
			})

			$("#news").append(newsRows);
		}
	}

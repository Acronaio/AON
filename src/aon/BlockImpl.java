...
final class BlockImpl implements Block {
    ...
    byte[] bytes() {
        if (bytes == null) {
            ...
            if (version < 3) {
                buffer.putInt((int) (totalAmountNQT / Constants.ONE_AON));
                buffer.putInt((int) (totalFeeNQT / Constants.ONE_AON));
            } else {
    ...
    void apply() {
        ...
        if (this.height > Constants.SHUFFLING_BLOCK) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                Logger.logDebugMessage("Back fees %f NXT to forger at height %d", ((double)backFees[i]) / Constants.ONE_AON, this.height - i - 1);
        ...
        if (totalBackFees != 0) {
            Logger.logDebugMessage("Fee reduced by %f NXT at height %d", ((double)totalBackFees) / Constants.ONE_AON, this.height);
        }

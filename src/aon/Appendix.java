...
import aon.crypto.EncryptedData;
...
import static aon.Constants.ONE_AON;
...
public interface Appendix {
    ...
    interface Prunable {
        ...
        default boolean shouldLoadPrunable(Transaction transaction, boolean includeExpiredPrunable) {
            return AON.getEpochTime() - transaction.getTimestamp() <
    ...
    class Message extends AbstractAppendix {
        ...
        private static final Fee MESSAGE_FEE = new Fee.SizeBasedFee(0, ONE_AON * Constants.FEE_MULTIPLIER, 32) {
            @Override
        ...
        void validate(Transaction transaction) throws NxtException.ValidationException {
            if (AON.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && message.length > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
    ...
    class PrunablePlainMessage extends Appendix.AbstractAppendix implements Prunable {
        ...
        private static final Fee PRUNABLE_MESSAGE_FEE = new Fee.SizeBasedFee(ONE_AON * Constants.FEE_MULTIPLIER / 10) {
            @Override
        ...
        void validate(Transaction transaction) throws NxtException.ValidationException {
            ...
            if (msg == null && AON.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
        ...
        @Override
        void apply(Transaction transaction, Account senderAccount, Account recipientAccount) {
            if (AON.getEpochTime() - transaction.getTimestamp() < Constants.MAX_PRUNABLE_LIFETIME) {
    ...
    abstract class AbstractEncryptedMessage extends AbstractAppendix {
        ...
        private static final Fee ENCRYPTED_MESSAGE_FEE = new Fee.SizeBasedFee(ONE_AON * Constants.FEE_MULTIPLIER, Constants.ONE_AON * Constants.FEE_MULTIPLIER, 32) {
            @Override
            public int getSize(TransactionImpl transaction, Appendix appendage) {
        ...
        @Override
        void validate(Transaction transaction) throws NxtException.ValidationException {
            if (AON.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && getEncryptedDataLength() > Constants.MAX_ENCRYPTED_MESSAGE_LENGTH) {
    ...
    class PrunableEncryptedMessage extends AbstractAppendix implements Prunable {
        ...
        private static final Fee PRUNABLE_ENCRYPTED_DATA_FEE = new Fee.SizeBasedFee(ONE_AON * Constants.FEE_MULTIPLIER / 10) {
            @Override
            public int getSize(TransactionImpl transaction, Appendix appendix) {
                return appendix.getFullSize();
            }
        ...
        void validate(Transaction transaction) throws NxtException.ValidationException {
            ...
            if (ed == null && AON.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
    ...
    final class PublicKeyAnnouncement extends AbstractAppendix {
        ...
        public PublicKeyAnnouncement(byte[] publicKey, byte transactionVersion) {
            super(transactionVersion);
            this.publicKey = publicKey;
        }
    ...
    final class Phasing extends AbstractAppendix {
        ...
        private static final Fee PHASING_FEE = (transaction, appendage) -> {
            ...
            if (phasing.hashedSecret.length > 0) {
                fee += (1 + (phasing.hashedSecret.length - 1) / 32) * ONE_AON;
            }
            fee += ONE_AON * phasing.linkedFullHashes.length;
            return fee;
        };
        ...
        void validate(Transaction transaction) throws NxtException.ValidationException {
            params.validate();
            int currentHeight = AON.getBlockchain().getHeight();
        ...
        private void release(TransactionImpl transaction) {
            ...
            AONTransactionEngineProcessor.getInstance().notifyListeners(Collections.singletonList(transaction), TransactionProcessor.Event.RELEASE_PHASED_TRANSACTION);
        ...
        void reject(TransactionImpl transaction) {
            ...
            AONTransactionEngineProcessor.getInstance()
        ...
        void countVotes(TransactionImpl transaction) {
            if (AON.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && PhasingPoll.getResult(transaction.getId()) != null) {
                return;
            }
            ...
            if (result >= poll.getQuorum()) {
                if (!transaction.attachmentIsDuplicate(duplicates, false)) {
                    try {
                        release(transaction);
                        poll.finish(result);
                        Logger.logDebugMessage("Early finish of transaction " + transaction.getStringId() + " at height " + AON.getBlockchain().getHeight());
                    } catch (RuntimeException e) {
                        Logger.logErrorMessage("Failed to release phased transaction " + transaction.getJSONObject().toJSONString(), e);
                    }
                } else {
                    Logger.logDebugMessage("At height " + AON.getBlockchain().getHeight() + " phased transaction " + transaction.getStringId()

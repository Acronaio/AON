...
import aon.AccountLedger.LedgerEntry;
import aon.AccountLedger.LedgerEvent;
import aon.AccountLedger.LedgerHolding;
import aon.crypto.Crypto;
import aon.crypto.EncryptedData;
import aon.db.DbClause;
import aon.db.DbIterator;
import aon.db.DbKey;
import aon.db.DbUtils;
import aon.db.DerivedDbTable;
import aon.db.VersionedEntityDbTable;
import aon.db.VersionedPersistentDbTable;
...
public final class Account {
    ...
    public static final class AccountAsset {
        ...
        private void save(Connection con) throws SQLException {
            try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO account_asset "
                ...
                pstmt.setInt(++i, AON.getBlockchain().getHeight());
                pstmt.executeUpdate();
            }
        }
    ...
    public static final class PublicKey {
        ...
        private PublicKey(long accountId, byte[] publicKey) {
            ...
            this.height = AON.getBlockchain().getHeight();
        }
        ...
        private void save(Connection con) throws SQLException {
            height = AON.getBlockchain().getHeight();
    ...
    private static final VersionedEntityDbTable<AccountAsset> accountAssetTable = new VersionedEntityDbTable<AccountAsset>("account_asset", accountAssetDbKeyFactory) {
        ...
        public void checkAvailable(int height) {
            if (height + Constants.MAX_DIVIDEND_PAYMENT_ROLLBACK < AON.getBlockchainProcessor().getMinRollbackHeight()) {
                throw new IllegalArgumentException("Historical data as of height " + height +" not available.");
            }
            if (height > AON.getBlockchain().getHeight()) {
                throw new IllegalArgumentException("Height " + height + " exceeds blockchain height " + AON.getBlockchain().getHeight());
            }
        }
    ...
    private static final ConcurrentMap<DbKey, byte[]> publicKeyCache = AON.getBooleanProperty("aon.enablePublicKeyCache") ?
    ...
    static {
        ...
        AON.getBlockchainProcessor().addListener(block -> {
        ...
        if (publicKeyCache != null) {
            ...
            AON.getBlockchainProcessor().addListener(block -> publicKeyCache.clear(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public long getEffectiveBalanceNXT() {
        return getEffectiveBalanceNXT(AON.getBlockchain().getHeight());
    }
    ...
    public long getEffectiveBalanceNXT(int height) {
        ...
        if (height < Constants.TRANSPARENT_FORGING_BLOCK_3) {
            if (Arrays.binarySearch(Genesis.GENESIS_RECIPIENTS, id) >= 0) {
                return balanceNQT / Constants.ONE_AON;
            }
            long receivedInLastBlock = 0;
            for (Transaction transaction : AON.getBlockchain().getBlockAtHeight(height).getTransactions()) {
                if (id == transaction.getRecipientId()) {
                    receivedInLastBlock += transaction.getAmountNQT();
                }
            }
            return (balanceNQT - receivedInLastBlock) / Constants.ONE_AON;
        }
        AON.getBlockchain().readLock();
        try {
            ...
            return (height > Constants.SHUFFLING_BLOCK && effectiveBalanceNQT < Constants.MIN_FORGING_BALANCE_NQT) ? 0 : effectiveBalanceNQT / Constants.ONE_AON;
        } finally {
            AON.getBlockchain().readUnlock();
        }
    }
    ...
    private long getLessorsGuaranteedBalanceNQT(int height) {
        ...
        int blockchainHeight = AON.getBlockchain().getHeight();
        try (Connection con = Db.db.getConnection();
    ...
    public long getGuaranteedBalanceNQT() {
        return getGuaranteedBalanceNQT(Constants.GUARANTEED_BALANCE_CONFIRMATIONS, AON.getBlockchain().getHeight());
    }
    ...
    public long getGuaranteedBalanceNQT(final int numberOfConfirmations, final int currentHeight) {
        ...
        try {
            ...
            if (height + Constants.GUARANTEED_BALANCE_CONFIRMATIONS < AON.getBlockchainProcessor().getMinRollbackHeight()
                    || height > AON.getBlockchain().getHeight()) {
    ...
    void leaseEffectiveBalance(long lesseeId, int period) {
        int height = AON.getBlockchain().getHeight();
    ...
    static boolean setOrVerify(long accountId, byte[] key) {
        ...
        if (publicKey.publicKey == null) {
            ...
            publicKey.height = AON.getBlockchain().getHeight();
            return true;
        }
    ...
    void apply(byte[] key) {
        ...
        } else if (publicKey.height >= AON.getBlockchain().getHeight() - 1) {

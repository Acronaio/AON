...
import aon.crypto.HashFunction;
import aon.http.API;
import aon.util.Convert;
import aon.util.Logger;
import aon.util.TrustAllSSLProvider;
...
public class MintWorker {
    ...
    private void mint() {
        String currencyCode = Convert.emptyToNull(AON.getStringProperty("aon.mint.currencyCode"));
        if (currencyCode == null) {
            throw new IllegalArgumentException("aon.mint.currencyCode not specified");
        }
        String secretPhrase = Convert.emptyToNull(AON.getStringProperty("aon.mint.secretPhrase", null, true));
        if (secretPhrase == null) {
            throw new IllegalArgumentException("aon.mint.secretPhrase not specified");
        }
        boolean isSubmitted = AON.getBooleanProperty("aon.mint.isSubmitted");
        boolean isStopOnError = AON.getBooleanProperty("aon.mint.stopOnError");
        ...
        String unitsStr = AON.getStringProperty("aon.mint.unitsPerMint");
        double wholeUnits = 1;
        ...
        long initialNonce = AON.getIntProperty("aon.mint.initialNonce");
        if (initialNonce == 0) {
            initialNonce = new Random().nextLong();
        }
        int threadPoolSize = AON.getIntProperty("aon.mint.threadPoolSize");
        if (threadPoolSize == 0) {
    ...
    private JSONObject mintImpl(String secretPhrase, long accountId, long units, long currencyId, byte algorithm,
        ...
        if (isSubmitted) {
            ...
            response.put("message", "aon.mint.isSubmitted=false therefore currency mint transaction is not submitted");
        }
        return response;
    }
    ...
    private JSONObject currencyMint(String secretPhrase, long currencyId, long nonce, long units, long counter) {
        ...
        Transaction.Builder builder = AON.newTransactionBuilder(Crypto.getPublicKey(secretPhrase), 0, Constants.ONE_AON,
    ...
    private JSONObject getJsonResponse(Map<String, String> params) {
        ...
        String host = Convert.emptyToNull(AON.getStringProperty("aon.mint.serverAddress"));
        if (host == null) {
            try {
        ...
        boolean useHttps = AON.getBooleanProperty("aon.mint.useHttps");
        if (useHttps) {
            protocol = "https";
            HttpsURLConnection.setDefaultSSLSocketFactory(TrustAllSSLProvider.getSslSocketFactory());
            HttpsURLConnection.setDefaultHostnameVerifier(TrustAllSSLProvider.getHostNameVerifier());
        }
        int port = Constants.isTestnet ? API.TESTNET_API_PORT : AON.getIntProperty("aon.apiServerPort");
        String urlParams = getUrlParams(params);
        URL url;
        try {
            url = new URL(protocol, host, port, "/aon" + urlParams);

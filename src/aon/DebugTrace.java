...
public final class DebugTrace {
    ...
    static final String QUOTE = AON.getStringProperty("aon.debugTraceQuote", "\"");
    static final String SEPARATOR = AON.getStringProperty("aon.debugTraceSeparator", "\t");
    static final boolean LOG_UNCONFIRMED = AON.getBooleanProperty("aon.debugLogUnconfirmed");

    static void init() {
        List<String> accountIdStrings = AON.getStringListProperty("aon.debugTraceAccounts");
        String logName = AON.getStringProperty("aon.debugTraceLog");
        ...
        AON.getBlockchainProcessor().addListener(block -> debugTrace.resetLog(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public static DebugTrace addDebugTrace(Set<Long> accountIds, String logName) {
        ...
        AON.getBlockchainProcessor().addListener(debugTrace::traceBeforeAccept, BlockchainProcessor.Event.BEFORE_BLOCK_ACCEPT);
        AON.getBlockchainProcessor().addListener(debugTrace::trace, BlockchainProcessor.Event.BEFORE_BLOCK_APPLY);
        AON.getTransactionProcessor().addListener(transactions -> debugTrace.traceRelease(transactions.get(0)), TransactionProcessor.Event.RELEASE_PHASED_TRANSACTION);
    ...
    private void traceShufflingCancel(Shuffling shuffling) {
        ...
        if (blamedAccountId != 0 && include(blamedAccountId)) {
            ...
            long generatorId = AON.getBlockchain().getLastBlock().getGeneratorId();
            if (include(generatorId)) {
                Map<String,String> generatorMap = getValues(generatorId, false);
                generatorMap.put("generation fee", String.valueOf(fee));
                generatorMap.put("event", "shuffling blame");
    ...
    private Map<String,String> lessorGuaranteedBalance(Account account, long lesseeId) {
        ...
        map.put("timestamp", String.valueOf(AON.getBlockchain().getLastBlock().getTimestamp()));
        map.put("height", String.valueOf(AON.getBlockchain().getHeight()));

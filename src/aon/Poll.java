...
import aon.db.EntityDbTable;
import aon.db.ValuesDbTable;
...
public final class Poll extends AbstractPoll {
    ...
    private static final boolean isPollsProcessing = AON.getBooleanProperty("aon.processPolls");
    ...
    public static DbIterator<Poll> getActivePolls(int from, int to) {
        return pollTable.getManyBy(new DbClause.IntClause("finish_height", DbClause.Op.GT, AON.getBlockchain().getHeight()), from, to);
    }
    ...
    public static DbIterator<Poll> getPollsByAccount(long accountId, boolean includeFinished, boolean finishedOnly, int from, int to) {
        ...
        if (finishedOnly) {
            dbClause = dbClause.and(new DbClause.IntClause("finish_height", DbClause.Op.LTE, AON.getBlockchain().getHeight()));
        } else if (!includeFinished) {
            dbClause = dbClause.and(new DbClause.IntClause("finish_height", DbClause.Op.GT, AON.getBlockchain().getHeight()));
        }
    ...
    public static DbIterator<Poll> searchPolls(String query, boolean includeFinished, int from, int to) {
        DbClause dbClause = includeFinished ? DbClause.EMPTY_CLAUSE : new DbClause.IntClause("finish_height", DbClause.Op.GT, AON.getBlockchain().getHeight());
    ...
    private Poll(Transaction transaction, Attachment.MessagingPollCreation attachment) {
        ...
        this.timestamp = AON.getBlockchain().getLastBlockTimestamp();
    }
    ...
    public boolean isFinished() {
        return finishHeight <= AON.getBlockchain().getHeight();
    }

    private List<OptionResult> countResults(VoteWeighting voteWeighting) {
        int countHeight = Math.min(finishHeight, AON.getBlockchain().getHeight());
        if (countHeight < AON.getBlockchainProcessor().getMinRollbackHeight()) {
            return null;
        }

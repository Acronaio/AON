...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && AON.getBlockchainProcessor().isScanning() ?
                    Math.max(AON.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
                    : AON.getBlockchainProcessor().getMinRollbackHeight();
    ...
    private boolean doesNotExceed(int height) {
        return AON.getBlockchain().getHeight() <= height && ! (isPersistent() && AON.getBlockchainProcessor().isScanning());
    }

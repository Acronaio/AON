 ...
 *
 *   java -cp "classes:lib/*:conf" aon.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Dnxt.runtime.mode=desktop aon.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize AON properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "aon.testDb" : "aon.db";
        String dbType = AON.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = AON.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = AON.getDbDir(AON.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = AON.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = AON.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = AON.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "aon.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "aon.mv.db");
            if (!dbFile.exists()) {

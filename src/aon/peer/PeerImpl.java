...
import aon.util.CountingInputReader;
import aon.util.CountingInputStream;
import aon.util.CountingOutputWriter;
...
final class PeerImpl implements Peer {
    ...
    void setVersion(String version) {
        ...
        isOldVersion = false;
        if (AON.APPLICATION.equals(application)) {
    ...
    public int getWeight() {
        ...
        if (hallmarkBalance == -1 || hallmarkBalanceHeight < AON.getBlockchain().getHeight() - 60) {
            long accountId = hallmark.getAccountId();
            Account account = Account.getAccount(accountId);
            hallmarkBalance = account == null ? 0 : account.getBalanceNQT();
            hallmarkBalanceHeight = AON.getBlockchain().getHeight();
        }
        return (int)(adjustedWeight * (hallmarkBalance / Constants.ONE_AON) / Constants.MAX_BALANCE_AON);
    }
    ...
    public void blacklist(String cause) {
        blacklistingTime = AON.getEpochTime();
    ...
    public JSONObject send(final JSONStreamAware request, int maxResponseSize) {
        ...
        try {
            ...
            if (useWebSocket && !webSocket.isOpen())
                useWebSocket = webSocket.startClient(URI.create("ws://" + host + ":" + getPort() + "/aon"));
            //
            ...
            if (useWebSocket) {
                ...
                //
                URL url = new URL("http://" + host + ":" + getPort() + "/aon");
                if (communicationLoggingMask != 0)
    ...
    void connect() {
        lastConnectAttempt = AON.getEpochTime();
        try {
    ...
    boolean analyzeHallmark(final String hallmarkString) {
        ...
        try {
            ...
            for (PeerImpl peer : groupedPeers) {
                peer.adjustedWeight = Constants.MAX_BALANCE_AON * peer.getHallmarkWeight(mostRecentDate) / totalWeight;

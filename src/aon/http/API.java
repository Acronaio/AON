...
import static aon.http.JSONResponses.MISSING_ADMIN_PASSWORD;
import static aon.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static aon.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
import static aon.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
...
public final class API {
    ...
    public static final String adminPassword = AON.getStringProperty("aon.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = AON.getIntProperty("aon.maxAPIRecords");
    static final boolean enableAPIUPnP = AON.getBooleanProperty("aon.enableAPIUPnP");
    public static final int apiServerIdleTimeout = AON.getIntProperty("aon.apiServerIdleTimeout");
    public static final boolean apiServerCORS = AON.getBooleanProperty("aon.apiServerCORS");
    private static final String forwardedForHeader = AON.getStringProperty("aon.forwardedForHeader");
    ...
    static {
        List<String> disabled = new ArrayList<>(AON.getStringListProperty("aon.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = AON.getStringListProperty("aon.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = AON.getStringListProperty("aon.allowedBotHosts");
        ...
        boolean enableAPIServer = AON.getBooleanProperty("aon.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : AON.getIntProperty("aon.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : AON.getIntProperty("aon.apiServerSSLPort");
            final String host = AON.getStringProperty("aon.apiServerHost");
            disableAdminPassword = AON.getBooleanProperty("aon.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = AON.getBooleanProperty("aon.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(AON.getUserHomeDir()).resolve(Paths.get(AON.getStringProperty("aon.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(AON.getStringProperty("aon.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(AON.getStringProperty("aon.keyStoreType"));
                List<String> ciphers = AON.getStringListProperty("aon.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = AON.getStringProperty("aon.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{AON.getStringProperty("aon.apiWelcomeFile")});
            }

            String javadocResourceBase = AON.getStringProperty("aon.javadocResourceBase");
            ...
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/aon");
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(AON.getIntProperty("aon.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));

            servletHolder = apiHandler.addServlet(APIProxyServlet.class, "/aon-proxy");
            ...
            if (!AON.getBooleanProperty("aon.enableAPIServerGZIPFilter", isOpenAPI)) {
                gzipHandler.setExcludedPaths("/aon", "/aon-proxy");
            }
            gzipHandler.setIncludedMethods("GET", "POST");
            gzipHandler.setMinGzipSize(aon.peer.Peers.MIN_COMPRESS_SIZE);
            ...
            if (AON.getBooleanProperty("aon.apiFrameOptionsSameOrigin")) {

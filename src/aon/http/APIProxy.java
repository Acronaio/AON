...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (AON.getBooleanProperty("aon.enableAPIProxy") && ! API.isOpenAPI);
    private static final int blacklistingPeriod = AON.getIntProperty("aon.apiProxyBlacklistingPeriod") / 1000;
    static final String forcedServerURL = AON.getStringProperty("aon.forceAPIProxyServerURL", "");
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && AON.getBlockchainProcessor().isDownloading());
    }
    ...
    boolean blacklistHost(String host) {
        ...
        blacklistedPeers.put(host, AON.getEpochTime() + blacklistingPeriod);

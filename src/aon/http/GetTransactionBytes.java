...
import static aon.http.JSONResponses.INCORRECT_TRANSACTION;
import static aon.http.JSONResponses.MISSING_TRANSACTION;
...
public final class GetTransactionBytes extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        transaction = AON.getBlockchain().getTransaction(transactionId);
        JSONObject response = new JSONObject();
        if (transaction == null) {
            transaction = AON.getTransactionProcessor().getUnconfirmedTransaction(transactionId);
            if (transaction == null) {
                return UNKNOWN_TRANSACTION;
            }
        } else {
            response.put("confirmations", AON.getBlockchain().getHeight() - transaction.getHeight());
        }

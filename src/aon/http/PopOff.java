...
public final class PopOff extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        try {
            AON.getBlockchainProcessor().setGetMoreBlocks(false);
            if (numBlocks > 0) {
                blocks = AON.getBlockchainProcessor().popOffTo(AON.getBlockchain().getHeight() - numBlocks);
            } else if (height > 0) {
                blocks = AON.getBlockchainProcessor().popOffTo(height);
            } else {
                return JSONResponses.missing("numBlocks", "height");
            }
        } finally {
            AON.getBlockchainProcessor().setGetMoreBlocks(true);
        }
        ...
        if (keepTransactions) {
            blocks.forEach(block -> AON.getTransactionProcessor().processLater(block.getTransactions()));
        }
        return response;
    }

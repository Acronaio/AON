...
import static aon.http.JSONResponses.ERROR_DISABLED;
import static aon.http.JSONResponses.ERROR_INCORRECT_REQUEST;
import static aon.http.JSONResponses.ERROR_NOT_ALLOWED;
import static aon.http.JSONResponses.LIGHT_CLIENT_DISABLED_API;
import static aon.http.JSONResponses.POST_REQUIRED;
import static aon.http.JSONResponses.REQUIRED_BLOCK_NOT_FOUND;
import static aon.http.JSONResponses.REQUIRED_LAST_BLOCK_NOT_FOUND;
...
public final class APIServlet extends HttpServlet {
    ...
    private static final boolean enforcePost = AON.getBooleanProperty("aon.apiServerEnforcePOST");
    private static final boolean fixResponseContentType = AON.getBooleanProperty("aon.apiFixResponseContentType");
    ...
    static {
        ...
        API.disabledAPIs.forEach(api -> {
            ...
            if (handler == null) {
                throw new RuntimeException("Invalid API in aon.disabledAPIs: " + api);
            }
        ...
        try {
            ...
            try {
                try {
                    ...
                    if (requireBlockId != 0 && !AON.getBlockchain().hasBlock(requireBlockId)) {
                        response = REQUIRED_BLOCK_NOT_FOUND;
                        return;
                    }
                    if (requireLastBlockId != 0 && requireLastBlockId != AON.getBlockchain().getLastBlock().getId()) {
                    ...
                    if (requireLastBlockId == 0 && requireBlockId != 0 && response instanceof JSONObject) {
                        ((JSONObject) response).put("lastBlock", AON.getBlockchain().getLastBlock().getStringId());
                    }
                } finally {
                    if (apiRequestHandler.startDbTransaction()) {

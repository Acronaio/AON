...
import static aon.http.JSONResponses.FEATURE_NOT_AVAILABLE;
import static aon.http.JSONResponses.INCORRECT_DEADLINE;
import static aon.http.JSONResponses.INCORRECT_EC_BLOCK;
import static aon.http.JSONResponses.INCORRECT_LINKED_FULL_HASH;
import static aon.http.JSONResponses.INCORRECT_WHITELIST;
import static aon.http.JSONResponses.MISSING_DEADLINE;
import static aon.http.JSONResponses.MISSING_SECRET_PHRASE;
import static aon.http.JSONResponses.NOT_ENOUGH_FUNDS;
...
abstract class CreateTransaction extends APIServlet.APIRequestHandler {
    ...
    private Appendix.Phasing parsePhasing(HttpServletRequest req) throws ParameterException {
        int finishHeight = ParameterParser.getInt(req, "phasingFinishHeight",
                AON.getBlockchain().getHeight() + 1,
                AON.getBlockchain().getHeight() + Constants.MAX_PHASING_DURATION + 1,
                true);
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
        ...
        if (ecBlockId != 0 && ecBlockId != AON.getBlockchain().getBlockIdAtHeight(ecBlockHeight)) {
            return INCORRECT_EC_BLOCK;
        }
        if (ecBlockId == 0 && ecBlockHeight > 0) {
            ecBlockId = AON.getBlockchain().getBlockIdAtHeight(ecBlockHeight);
        }
        ...
        try {
            Transaction.Builder builder = AON.newTransactionBuilder(publicKey, amountNQT, feeNQT,
            ...
            if (broadcast) {
                AON.getTransactionProcessor().broadcast(transaction);

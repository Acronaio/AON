...
final class TransactionImpl implements Transaction {
    ...
    private TransactionImpl(BuilderImpl builder, String secretPhrase) throws NxtException.NotValidException {
        ...
        if (builder.feeNQT <= 0 || (Constants.correctInvalidFees && builder.signature == null)) {
            int effectiveHeight = (height < Integer.MAX_VALUE ? height : AON.getBlockchain().getHeight());
    ...
    DbKey getDbKey() {
        if (dbKey == null) {
            dbKey = AONTransactionEngineProcessor.getInstance().unconfirmedTransactionDbKeyFactory.newKey(getId());
        }
        return dbKey;
    }
        ...
        if (bytes == null) {
            try {
                ...
                if (useNQT()) {
                    ...
                    buffer.putInt((int) (amountNQT / Constants.ONE_AON));
                    buffer.putInt((int) (feeNQT / Constants.ONE_AON));
                    if (referencedTransactionFullHash != null) {
    ...
    private boolean useNQT() {
        return this.height > Constants.NQT_BLOCK
                ...
                || AON.getBlockchain().getHeight() >= Constants.NQT_BLOCK);
    }
        ...
        if (!validatingAtFinish) {
            ...
            if (feeNQT < minimumFeeNQT) {
                throw new NxtException.NotCurrentlyValidException(String.format("Transaction fee %f NXT less than minimum fee %f NXT at height %d",
                        ((double) feeNQT) / Constants.ONE_AON, ((double) minimumFeeNQT) / Constants.ONE_AON, blockchainHeight));
            }
    ...
    private long getMinimumFeeNQT(int blockchainHeight) {
        ...
        if (referencedTransactionFullHash != null) {
            totalFee = Math.addExact(totalFee, Constants.ONE_AON);
        }
        return totalFee;
    }

...
public final class Shuffler {
    ...
    private static final int MAX_SHUFFLERS = AON.getIntProperty("aon.maxNumberOfShufflers");
    ...
    static {
        ...
        AONEngineBlockchainProcessor.getInstance().addListener(block -> {
        ...
        AONEngineBlockchainProcessor.getInstance().addListener(block -> shufflingsMap.values().forEach(shufflerMap -> shufflerMap.values().forEach(shuffler -> {
            if (shuffler.failedTransaction != null) {
                try {
                    AONTransactionEngineProcessor.getInstance().broadcast(shuffler.failedTransaction);
        ...
        AONEngineBlockchainProcessor.getInstance().addListener(block -> stopAllShufflers(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    private static void scheduleExpiration(Shuffling shuffling) {
        int expirationHeight = AON.getBlockchain().getHeight() + 720;
    ...
    private void submitTransaction(Attachment.ShufflingAttachment attachment) {
        if (AONEngineBlockchainProcessor.getInstance().isProcessingBlock()) {
            if (hasUnconfirmedTransaction(attachment, AONTransactionEngineProcessor.getInstance().getWaitingTransactions())) {
                Logger.logDebugMessage("Transaction already submitted");
                return;
            }
        } else {
            try (DbIterator<UnconfirmedTransaction> unconfirmedTransactions = AONTransactionEngineProcessor.getInstance().getAllUnconfirmedTransactions()) {
        ...
        try {
            Transaction.Builder builder = AON.newTransactionBuilder(Crypto.getPublicKey(secretPhrase), 0, 0,
                    (short) 1440, attachment);
            builder.timestamp(AON.getBlockchain().getLastBlockTimestamp());
            ...
            try {
                AONTransactionEngineProcessor.getInstance().broadcast(transaction);

...
package aon.http.accountControl;

import aon.BlockchainTest;
import aon.Constants;
import aon.AON;
import aon.PhasingParams;
import aon.VoteWeighting.MinBalanceModel;
import aon.VoteWeighting.VotingModel;
import aon.http.APICall;
import aon.http.APICall.Builder;
...
public class PhasingOnlyTest extends BlockchainTest {
    ...
    public void testAccountVoting() throws Exception {
        ...
        Builder builder = new ACTestUtils.Builder("sendMoney", ALICE.getSecretPhrase())
                .recipient(BOB.getId())
                .param("amountNQT", 1 * Constants.ONE_AON);
    ...
    public void testExtraRestrictions() throws Exception {
        ...
        setPhasingOnlyControl(VotingModel.ACCOUNT, null, 1L, null, null, new long[] {BOB.getId(), CHUCK.getId()},
                5 * Constants.ONE_AON, 4, 100);

        Builder builder = new ACTestUtils.Builder("sendMoney", ALICE.getSecretPhrase())
                .recipient(BOB.getId())
                .param("amountNQT", 1 * Constants.ONE_AON)
                .feeNQT(7 * Constants.ONE_AON);
        // fee too high
        setTransactionPhasingParams(builder, 20, VotingModel.ACCOUNT, null, 1L, null, null, new long[] {BOB.getId(), CHUCK.getId()});
        ACTestUtils.assertTransactionBlocked(builder);

        // fee at the limit
        builder.feeNQT(5 * Constants.ONE_AON);
        ...
        builder.param("phasingFinishHeight", AON.getBlockchain().getHeight() + 200);
        ACTestUtils.assertTransactionBlocked(builder);
        builder.param("phasingFinishHeight", AON.getBlockchain().getHeight() + 3);
        ACTestUtils.assertTransactionBlocked(builder);
        builder.param("phasingFinishHeight", AON.getBlockchain().getHeight() + 4);
        ACTestUtils.assertTransactionSuccess(builder);
    ...
    public void testRejectingPendingTransaction() throws Exception {
        ...
        Assert.assertEquals(balanceBeforeTransactionRejection + 1 * Constants.ONE_AON,
                ACTestUtils.getAccountBalance(ALICE.getId(), "unconfirmedBalanceNQT"));
    }

    @Test
    public void testBalanceVoting() {
        setPhasingOnlyControl(VotingModel.NQT, null, 100 * Constants.ONE_AON, null, null, null, 0, 0, 0);
        ...
        setTransactionPhasingParams(builder, 20, VotingModel.NQT, null, 100 * Constants.ONE_AON, null, null, new long[] {DAVE.getId()});
        ACTestUtils.assertTransactionBlocked(builder);

        setTransactionPhasingParams(builder, 20, VotingModel.ACCOUNT, null, 1L, null, null, new long[] {BOB.getId(), CHUCK.getId()});
        ACTestUtils.assertTransactionBlocked(builder);

        setTransactionPhasingParams(builder, 20, VotingModel.NQT, null, 100 * Constants.ONE_AON + 1, null, null, null);
        ACTestUtils.assertTransactionBlocked(builder);
        ...
        setTransactionPhasingParams(builder, 20, VotingModel.NQT, null, 100 * Constants.ONE_AON, null, null, null);
        ACTestUtils.assertTransactionSuccess(builder);
    }
    ...
    private void setTransactionPhasingParams(Builder builder, int finishAfter, VotingModel votingModel, String holdingId, Long quorum,
        ...
        builder.param("phasingFinishHeight", AON.getBlockchain().getHeight() + finishAfter);

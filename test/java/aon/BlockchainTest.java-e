...
public abstract class BlockchainTest extends AbstractBlockchainTest {
    ...
    public static void initNxt() {
        if (!isNxtInitialized) {
            ...
            properties.setProperty("aon.fakeForgingAccount", forgerAccountId);
            properties.setProperty("aon.timeMultiplier", "1");
            properties.setProperty("aon.testnetGuaranteedBalanceConfirmations", "1");
            properties.setProperty("aon.testnetLeasingDelay", "1");
            properties.setProperty("aon.disableProcessTransactionsThread", "true");
    ...
    public static void generateBlock() {
        try {
            blockchainProcessor.generateBlock(forgerSecretPhrase, AON.getEpochTime());
